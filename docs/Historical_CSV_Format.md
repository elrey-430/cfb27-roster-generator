...
## The announcers say your players' names

Nothing to fill in — this happens on its own, from the `LastName` you already
supply.

The game stores a **commentary index** per player, choosing which recorded
name the announcers use. Until now a recreated player kept whatever the roster
slot already had, so your Jordan Travis was called by the name of the player
he replaced, all season, with nothing in the game to tell you.

Each player's surname now sets that index, exactly as the game does it when
you rename someone in its own editor. A surname the commentary has no
recording of gets **0**, which means the announcers simply do not say the name
— never somebody else's. On the 2023 Florida State roster that is 61 of 85
players named and 24 left unnamed, and the report gives you those counts.

The mapping covers **5,918 surnames**, measured from 146,295 player rows across
nine dynasty saves the game generated itself. It lives in
`data/CommentaryIds.json` and is regenerated by
`tools/build_commentary_ids.py`.
...
| LastName (again) | `PLYR_COMMENT` — the recorded name the announcers say, or 0 when there is no recording of it |
